@section('title', 'Edit Post ' . $post->post_id)
...
<div class="container">
        ...
        <div class="space-y-8 divide-y divide-gray-200 mt-10">
            ...
            <form wire:submit.prevent="savePost" enctype="multipart/form-data">
                <div class="sm:col-span-6 m-2">
                    ...
                    <div class="mt-1">
                        <input type="file" id="post_image" wire:model="post_image" name="post_image"
                               class="block w-full transition duration-150 ease-in-out appearance-none bg-white border border-gray-400 rounded-md py-2 px-3 text-base leading-normal transition duration-150 ease-in-out sm:text-sm sm:leading-5">
                    </div>
                    <div wire:loading wire:target="post_image">Uploading...</div>

                    @error('post_image') <span class="error">{{ $message }}</span> @enderror
                ...
                <div class="sm:col-span-6 m-2">
                    ...
                    <div class="mt-1">
                        <input id="title" wire:model="post_title" name="post_title" value="{{$post_title}}"
                               class="block w-full transition duration-150 ease-in-out appearance-none bg-white border border-gray-400 rounded-md py-2 px-3 text-base leading-normal transition duration-150 ease-in-out sm:text-sm sm:leading-5">
                    </div>
                    ...
                    <div class="mt-1">
                        <select wire:model="post_category" name="post_category" id="post_category"
                            ...
                            @foreach($category->all() as $row)
                                <option
                                    {{$row->category_id === $post->category_id ? "selected" : ""}} value="{{$row->category_id}}">{{$row->category_title}}</option>
                            @endforeach
                        </select>
                    </div>
                    @error('post_category') <span class="error">{{ $message }}</span> @enderror
                </div>
                <div class="sm:col-span-6 m-2 ">
    ...
    <span class="ql-formats">
      ...
      <button class="ql-clean"></button>
                            ...
                            <div class="h-64" x-data x-ref="quillEditor"
                                 x-init="quill = new Quill($refs.quillEditor, {theme: 'bubble' , modules: {toolbar: '#toolbar-container'}});quill.root.innerHTML = '{!! $post_content !!}';quill.on('text-change', function () {$dispatch('quill-input', quill.root.innerHTML);});"
