...
<div class="container">
    <div class="w-2/4 mx-auto p-4">
        <h1 class="text-4xl mt-6 tracking-tight leading-10 font-extrabold text-gray-900 sm:text-5xl sm:leading-none md:text-6xl">
            Create Category</h1>
        <p class="text-lg mt-2 text-gray-600">Start crafting your new category below.</p>
        <div class="space-y-8 divide-y divide-gray-200 mt-10">
            ...
            @if($saveSuccess)
                <div class="rounded-md bg-green-100 rounded-lg p-4">
                    <div class="flex">
                        ...
                        <div class="ml-3">
                            <h3 class="text-sm font-medium text-green-800">Successfully Saved Category</h3>
                            <div class="mt-2 text-sm text-green-700">
                                <p>Your new category has been saved.</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <form wire:submit.prevent="saveCategory" enctype="multipart/form-data">
                <div class="sm:col-span-6 m-2">
                    ...
                    <div class="mt-1">
                        <input id="title" wire:model="category_title" name="category_title"
                               class="block w-full transition duration-150 ease-in-out appearance-none bg-white border border-gray-400 rounded-md py-2 px-3 text-base leading-normal transition duration-150 ease-in-out sm:text-sm sm:leading-5">
                    </div>
                    @error('category_title') <span class="error">{{ $message }}</span> @enderror
    ...
    toolbar: [
      ...
      "
                                x-on:quill-input.debounce.2000ms="@this.set('category_content', $event.detail)"
                            >
                                {!! $category_content !!}
                            </div>
                        </div>
                    </div>
                    @error('category_content') <span class="error">{{ $message }}</span> @enderror

                </div>
                <div wire:click="saveCategory"
